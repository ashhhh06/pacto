...
import { 
  ...
  Sparkles, Calendar, Bell, Filter, X
} from 'lucide-react';
...
export default function ObligationTracker() {
  ...
  const [newDueDate, setNewDueDate] = useState(new Date(Date.now() + 30*24*60*60*1000).toISOString().split('T')[0]);
  const [newPriority, setNewPriority] = useState('High');

  const allObligations = contracts.flatMap(c => 
    (c.obligations || []).map(ob => ({ ...ob, contractTitle: c.title, contractId: c.id || c._id, client: c.client }))
  );
  ...
  const handleCreateObligation = async (e) => {
    e.preventDefault();
    if (!newTask.trim() || !selectedContractId) return;

    await addObligation(selectedContractId, newTask, newOwner, newDueDate, newPriority);
    setNewTask('');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-semibold">
            ...
            <span>Obligation & Deliverables Tracker</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white mt-1">
            Obligation & Deliverables Tracker
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Track contractual commitments, assigned task owners, due dates, and completion status.
          </p>
        </div>
        ...
        {contracts.length > 0 && (
          <button
            onClick={() => {
              if (!selectedContractId) setSelectedContractId(contracts[0]?.id || contracts[0]?._id);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl glass-panel bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1 shadow-sm">
          <span className="text-xs font-semibold text-slate-500">Total Contract Obligations</span>
          <p className="text-2xl font-extrabold font-mono text-slate-900 dark:text-white">{allObligations.length}</p>
          <span className="text-[10px] text-blue-500 font-semibold">Tracked Across {contracts.length} Agreements</span>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1 shadow-sm">
          ...
          <span className="text-[10px] text-slate-400">Requires Owner Execution</span>
        </div>

        <div className="p-5 rounded-2xl glass-panel bg-slate-900 text-white space-y-1 shadow-sm">
          ...
          <span className="text-[10px] text-emerald-400 font-semibold">Compliance Verified</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {allObligations.length === 0 ? (
          <div className="glass-panel rounded-3xl p-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 shadow-xl">
            <CheckSquare className="w-10 h-10 text-blue-500 mx-auto opacity-70" />
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Obligation Tasks Found</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Upload a contract or create a new obligation task to track legal and operational commitments.
            </p>
          </div>
        ) : (
            ...
            {filteredObligations.map((ob) => (
              ...
              >
                <div className="flex items-center space-x-3.5">
                  <button
                    onClick={() => toggleObligationStatus(ob.contractId, ob.id || ob._id)}
                    className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${
                      ob.status === 'Completed' ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 dark:border-slate-600 hover:border-blue-500'
                    }`}
                  >
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-slate-500">
                      Contract: <strong className="text-blue-600 dark:text-blue-400">{ob.contractTitle}</strong>
                    </p>
                  </div>
                </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fadeIn">
          <form onSubmit={handleCreateObligation} className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="e.g. Execute Annual Data Privacy Compliance Review"
